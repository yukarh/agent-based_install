...
本手順の主な到達点は、オンプレ閉域想定の Agent-based Installer 入力作成、ミラー構成、`install-config.yaml` / `agent-config.yaml` 作成、Agent ISO 生成までとする。実ベアメタルでの ISO 起動、RHCOS インストール、`openshift-install-fips agent wait-for install-complete` による完了確認は次フェーズで扱う。
...
| `UPSTREAM_DNS` | work の BIND が必要に応じて参照する上位DNS。完全閉域で不要なら空 | AWS模擬では `10.0.0.2` |
| `UPSTREAM_NTP` | work の chrony が参照する上位NTP。完全閉域で不要なら空 | AWS模擬では `169.254.169.123` |
| `OC_MIRROR_VERSION` | `latest` で確認した oc-mirror の実バージョン。検証済み値を固定して使う | `4.22.x` など |
...
# AWS模擬では VPC DNS Resolver / Amazon Time Sync を使う。
# 実オンプレ閉域では社内DNS/NTPへ変更する。完全閉域で上位DNS/NTPを使わない場合は空にする。
...
DNS / NTP の上位参照先は環境依存である。本手順のデフォルト値はAWS模擬用だが、実オンプレ閉域では `UPSTREAM_DNS` / `UPSTREAM_NTP` を社内DNS/NTPに置き換える。完全閉域で上位参照を使わない場合は空にする。
...
# SNO の逆引きは、少なくともノード名 master-0.ocp.lab.test が返ることを確認する。
dig -x ${SNO_PRIVATE_IP} +short | grep -Fx "${SNO_HOSTNAME}."
...
# oc-mirror は latest をそのまま使うと、後日の再実行で挙動差分や既知不具合を踏むことがある。
# latest は「その時点の実バージョン確認」に使い、確認したバージョンを固定URLで取得する。
OC_MIRROR_LATEST_URL="https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/latest/oc-mirror.rhel9.tar.gz"

OC_MIRROR_VERSION="$(
  curl -fsSIL "${OC_MIRROR_LATEST_URL}" |
    awk 'BEGIN{IGNORECASE=1} /^location:/ {print $2}' |
    tr -d '\r' |
    sed -n 's#.*clients/ocp/\([^/]*\)/oc-mirror.rhel9.tar.gz#\1#p' |
    tail -n 1
)"

if [ -z "${OC_MIRROR_VERSION}" ]; then
  echo "latest から oc-mirror の実バージョンを判定できなかった。検証済みバージョンを手動指定する。"
  echo '例: export OC_MIRROR_VERSION="4.22.x"'
  exit 1
fi

echo "OC_MIRROR_VERSION=${OC_MIRROR_VERSION}"
curl -fL -O "https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/${OC_MIRROR_VERSION}/oc-mirror.rhel9.tar.gz"
...
### 7.1 oc-mirror のバージョンを固定して記録する

`oc-mirror` は OpenShift 本体の `OCP_VERSION` と同一とは限らない。今回の検証では、`latest` の実体を確認し、その時点のバージョンを固定URLで取得する運用にする。再実行時は、ここで表示された `OC_MIRROR_VERSION` と `oc-mirror version` の出力をメモしておく。
...
実ベアメタルで複数ディスクが見える場合は、誤ったディスクへ RHCOS を書き込まないよう、必要に応じて `rootDeviceHints` を追加する。今回のAWS模擬手順では単一ルートディスク前提のため省略する。
...
実ベアメタルでの ISO 起動、RHCOS インストール、`openshift-install-fips agent wait-for install-complete --dir iso-create` による完了確認は、本手順の次フェーズで扱う。
...
実オンプレのベアメタルで複数ディスクが見える場合は、`agent-config.yaml` に `rootDeviceHints` を追加するか検討する。単一ディスク前提の検証では省略している。

`oc-mirror` は `latest` を直接ダウンロードして使うのではなく、`latest` の実体バージョンを確認し、その時点の検証済みバージョンを固定URLで取得する。再実行時は `OC_MIRROR_VERSION` を記録する。
